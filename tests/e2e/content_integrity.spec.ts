...
test.describe('Content Integrity & Proactive UI', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/v1/tours/specialty/next?type=full_moon_party', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page.route('**/api/v1/tours/available?tour_date=2026-05-31', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([
    ...
    await page.route('**/api/v1/tours/available?tour_date=**', async (route) => {
    ...
    const bookNowButton = page.getByRole('link', { name: /Book Now|Reservar Agora/i }).first();
    if (await bookNowButton.isVisible()) {
        await bookNowButton.click();
    }
  });

  test('Pillar A: Form input text visibility (Ghost Text Fix)', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /Book Now|Reservar Agora/i }).first().click();
  ...
  test('Pillar B: Next Full Moon Discovery Banner visibility and behavior', async ({ page }) => {
