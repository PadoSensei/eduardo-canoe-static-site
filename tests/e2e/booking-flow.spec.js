...
test.describe("Money Loop Smoke Test", () => {
  test("User can complete a booking from start to finish @smoke", async ({
    ...
    // The button should stay disabled for at least 1000ms due to ShieldedButton cooldown.
    // We use Promise.all to catch the state as soon as the click is initiated.
    await Promise.all([
      confirmButton.click(),
      expect(confirmButton).toBeDisabled().catch(() => {
        // Fallback: If it transitions too fast, we accept it being hidden/missing
        return true;
      })
    ]);
