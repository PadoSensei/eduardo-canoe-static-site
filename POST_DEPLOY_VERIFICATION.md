...
import("./src/core/config.ts").then(m => {
  const config = m.default;
  console.log("%c --- Environment Forensic Audit ---", "color: #10b981; font-weight: bold;");
  ...
  console.log("Note: Direct import failed (expected in production). Please check the Network tab for API request URLs.");
});
```
